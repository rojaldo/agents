#!/usr/bin/env python3
"""
LoRA Training Demo for Language Models
Demonstrates the full training pipeline with a smaller model
"""
...
# Configuration - usando modelo mas pequeno para demo
CONFIG = {
    "model_name": "gpt2",  # Modelo demo de 117M (cabe facilmente)
    "max_seq_length": 512,
    "lora_rank": 8,
    "lora_alpha": 16,
    "lora_dropout": 0.05,
...
def main():
    print("=" * 70)
    print("LoRA Training Demo - Constitutional Data Fine-Tuning")
    print("=" * 70)
    ...
    if not text_data:
        ...
        print("Please ensure you have .md, .txt, or .json files in the data folder")
        return
    ...
    # Step 4: Load model
    print("\n[4/5] Loading GPT2 model for demo...")
    try:
        model = AutoModelForCausalLM.from_pretrained(
            CONFIG["model_name"],
            trust_remote_code=True,
        )
        print(f"Model loaded: {CONFIG['model_name']} ({sum(p.numel() for p in model.parameters())/1e6:.1f}M parameters)")
    except Exception as e:
    ...
    training_args = TrainingArguments(
        ...
        save_strategy="no",  # No guardar puntos intermedios
        logging_steps=1,
        gradient_accumulation_steps=1,
        report_to=[],  # Sin reporting externo
    )
    ...
    print(f"  model = AutoPeftModelForCausalLM.from_pretrained('{CONFIG['output_dir']}')")
    print("=" * 70)
